...
// –– وسوم الشعراء: رموز هندسية مستوحاة من فكرة “الوسم” الذي يُستخدم
// لتمييز كل شاعر داخل الموقع (بطاقات، فلاتر، صفحة القصيدة) ––
const WASM_ICONS = {
...
el.poemsGrid.innerHTML = `<div class="empty-state"> تعذّر تحميل بيانات الديوان (data/diwan.json).<br> إذا كنت تفتح الملف مباشرة من جهازك، شغّل خادمًا محليًا بسيطًا (مثل <code>python3 -m http.server</code>) ثم افتح الموقع من المتصفح، أو ارفع الملفات كلها على GitHub Pages. </div>`;
return;
}
...
el.wasmLegend.innerHTML = state.data.poets.map(poet => `<button class="wasm-legend-item" data-poet="${poet.id}" aria-label="عرض قصائد ${poet.name}"> ${wasmIcon(poet.wasm)} <span>${poet.name}</span> </button>`).join(””);
}

function renderFilterPills() {
const poetPills = state.data.poets.map(poet => `<button class="pill" data-poet="${poet.id}"> ${wasmIcon(poet.wasm, "width:16px;height:16px;")} ${poet.name} </button>`).join(””);
...
// بانر تعريف الشاعر عند اختيار شاعر معيّن
...
const cards = items.map(({ poet, poem }) => `<article class="poem-card" data-poem="${poem.id}" tabindex="0" role="button"> <div class="poem-card-tag">${wasmIcon(poet.wasm, "width:14px;height:14px;")} ${poet.name}</div> <h3>${poem.title}</h3> <p>${(poem.verses[0] ? poem.verses[0].sadr : "")}</p> </article>`).join(””);

el.poemsGrid.innerHTML = bioBanner + `<div class="poems-grid" style="display:contents">${cards}</div>`;
}
...
if (!poet) return “”;
return `<div class="poet-bio-banner" style="grid-column:1/-1"> ${wasmIcon(poet.wasm)} <div> <h2>${poet.name}</h2> <p>${poet.bio}</p> </div> </div>`;
}
...
const versesHtml = poem.verses.map(v => `<div class="verse"> <span class="sadr">${v.sadr}</span> <span class="divider"></span> <span class="ajz">${v.ajz}</span> </div>`).join(””);

el.poemDetail.innerHTML = `<button class="back-btn" data-return-to="${poet.id}"> → الرجوع إلى قصائد ${poet.name} </button> <div class="poem-header"> ${wasmIcon(poet.wasm)} <h2>${poem.title}</h2> <div class="poem-meta">${poet.name}${poem.date ? " · " + poem.date : ""}${poem.meter ? " · " + poem.meter : ""}</div> </div> <div class="verses">${versesHtml}</div>`;
}
