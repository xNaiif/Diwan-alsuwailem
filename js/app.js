...
const WASM_ICONS = {
  "wasm-1": `<svg viewBox="0 0 40 40" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round">
      <line x1="20" y1="6" x2="20" y2="34" />
      <line x1="11" y1="11" x2="29" y2="11" />
    </svg>`,
  "wasm-2": `<svg viewBox="0 0 40 40" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round">
      <circle cx="20" cy="20" r="9" />
      <line x1="5" y1="20" x2="35" y2="20" />
    </svg>`,
  "wasm-3": `<svg viewBox="0 0 40 40" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round">
...
async function init() {
  ...
  try {
    ...
    el.poemsGrid.innerHTML = `<div class="empty-state">
      تعذّر تحميل بيانات الديوان (data/diwan.json).<br>
      إذا كنت تفتح الملف مباشرة من جهازك، شغّل خادمًا محليًا بسيطًا
      (مثل <code>python3 -m http.server</code>) ثم افتح الموقع من المتصفح،
      أو ارفع الملفات كلها على GitHub Pages.
    </div>`;
    return;
  }
...
function renderBioBanner() {
  ...
  if (!poet) return "";
  return `
    <div class="poet-bio-banner" style="grid-column:1/-1">
      ${wasmIcon(poet.wasm)}
      <div>
        <h2>${poet.name}</h2>
        <p>${poet.bio}</p>
...
function showPoem(poemId) {
  ...
  el.poemDetail.innerHTML = `
    <button class="back-btn" data-return-to="${poet.id}">
      → الرجوع إلى قصائد ${poet.name}
    </button>
